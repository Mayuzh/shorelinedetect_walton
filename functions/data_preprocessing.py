...
def load_images(imagePaths,targetSize,edgeCoords=False):
    ...
    print('reading image, size info:', infoI.shape)
...
def load_single_image(image, targetSize):
    # Process a single image (frame) and transform it to the correct size

    # Convert the image to RGB
    origI = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    # Check the orientation of the image and rotate if needed
    if origI.shape[0] > origI.shape[1]:
        origI = cv2.rotate(origI, cv2.ROTATE_90_CLOCKWISE)

    # Get the crop and scale information
    cropInfo = get_scale_info(origI.shape, targetSize)

    # Crop before resize if needed
    if cropInfo['cropBool']:
        if cropInfo['cropHWBool']:
            cropDiff = int((origI.shape[0] - cropInfo['cropHW']) / 2)
            cropI = origI[cropDiff:-cropDiff, ...]
        else:
            cropDiff = int((origI.shape[1] - cropInfo['cropHW']) / 2)
            cropI = origI[:, cropDiff:-cropDiff, :]
        origI = cropI.copy()
