...
def hed_predict_single(model, image):
    '''
    This gives the final predictions out (with sigmoid activation) for a single image
    '''
    # Forward pass through the model
    d1, d2, d3, d4, d5, d6 = model.forward(image)  # No need to add another batch dimension
    ...
    # Concatenate and unsqueeze to add the singleton dimension
    allout = torch.cat([d1, d2, d3, d4, d5, d6], 1)
...
class hed_cnn(nn.Module):
    ...
    def forward(self, x):
        ...
        d1 = self.dsn1(conv1)
        d2 = F.interpolate(self.dsn2(conv2), size=(h, w), mode='nearest')
        d3 = F.interpolate(self.dsn3(conv3), size=(h, w), mode='nearest')
        d4 = F.interpolate(self.dsn4(conv4), size=(h, w), mode='nearest')
        d5 = F.interpolate(self.dsn5(conv5), size=(h, w), mode='nearest')
